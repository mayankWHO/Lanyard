...
function Card({ item, dimmed, onHoverStart, onHoverEnd, className }: CardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex h-full flex-col gap-5 overflow-hidden rounded-2xl border p-6",
        // Light
...
export default function SpotlightCards({
  ...
  return (
    ...
    >
      ...
      <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-[180px]">
        {items.map((item) => (
          <div key={item.title} className={item.gridClass}>
            <Card
